...
### git 동기화 규칙
- `git pull` / `git push` 요청 시 **확인 없이 바로 실행** — "할까요?" 묻지 말 것
- 연동 확인 요청 시 `git fetch` → 상태 파악 → 필요하면 바로 pull까지 실행
...
- **수정·버그픽스·기능 구현 등 작업이 끝날 때마다 메모리 파일(`project_moida_otpfc.md`)을 자동 업데이트** — 사용자가 요청하지 않아도, 언제 /compact를 해도 다음 세션에서 맥락이 유지되도록
...
| 상태 | 체크인 시간 | 계산식 |
|------|------------|--------|
| 정상 | 06:50:00 ~ 07:51:00 | `allowFrom(시작-70분)` ~ `normalThreshold(시작-9분, 10분+1분 여유)` |
...
- `allowFrom = meetingStart - 70분` (체크인 창 열림 — 8:00 시작 기준 06:50 오픈)
- `normalThreshold = meetingStart - 9분` (정상/지각 경계 — 07:51. "10분전 마감 + 1분 여유"로 start-9분 사용)
- `meetingEnd` 이후 수동 체크인 불가 (자동 노쇼 처리 완료)
- meetingTimes.start = 실제 경기 시작 시간 (08:00). 매치 테이블도 이 값 기준 2시간 계산
