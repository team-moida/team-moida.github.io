...
| 상태 | 체크인 시간 | 계산식 |
|------|------------|--------|
| 정상 | 07:50:00 ~ 08:00:00 | `allowFrom(시작-10분)` ~ `meetingStart` |
| 지각 | 08:00:01 ~ 10:00:00 | `meetingStart` 초과 ~ `meetingEnd` |
| 노쇼 | 미체크인 | 모임 종료 시 자동 처리 |
| 노쇼(1만원) | 전날 22:00 이후 통보 | 운영진 수동 설정 |
| 노쇼(2만원) | 당일 00:00~모임 직전 통보 | 운영진 수동 설정 |

- `allowFrom = meetingStart - 10분` (체크인 창 열림 — 8:00 시작 기준 07:50 오픈)
- `normalThreshold = meetingStart` (시작 전 체크인=정상, 시작 이후=지각)
- `meetingEnd` 이후 수동 체크인 불가 (자동 노쇼 처리 완료)
- meetingTimes.start = 실제 경기 시작 시간 (08:00). 매치 테이블도 이 값 기준 2시간 계산
- 테스트 모드: 시작 12분 후, 종료 시작+5분 → 시작 전=정상, 시작 이후=지각
