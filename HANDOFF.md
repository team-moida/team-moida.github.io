...
**마지막 갱신:** 2026-07-02 (저녁, PC) · **SW 캐시:** `moida-v380` · **표시버전:** `v1.26.9` · **마지막 커밋:** `f479eed`(+ 이 HANDOFF 커밋)
...
## 오늘 저녁 세션 (2026-07-02, PC, v1.24.1 → v1.26.9, 배포됨) — 모임 탭 회원/운영진 개편 이어서
회원=열람 / 운영진=관리 방향. LAB 먼저 보여주고 승인 후 적용하는 방식으로 진행.
- **회원 모임 상세 = 단일 스크롤 페이지**(홈 톤): 모임 카드 누르면 일시·장소·내 신청여부(신청/취소/불참·노쇼)·팀·번호·매치표를 한 페이지에서. → tab-attend.js(`MemberMeetingDetail`), MemberMeetingList가 `selId`로 상세 전환.
- **매치표 '아직 안 만듦' 버그**: 홈과 다른 소스를 봐서 항상 '안 만듦'으로 뜸 → `scheduleData.schedule.list` 기준으로 통일. → member.html(matchStatus).
- **명단/팀 헤더 개편**: 헤더 아래 중복 날짜 제거 · **담당자 이름+[변경]을 헤더(일시·장소)로** · **선정 인원(n/max)·모집중/마감을 헤더 우측 상단**으로 · 명단/팀 토글 좌측 축소 · 선착순 경고 박스 제거·축소. → tab-attend.js(`MeetingDetailHeader`), member.html.
- **선정 명단 = 정사각 카드 그리드**: 등급 제거(팀편성에서만) · 이름 중앙·박스크기 유동(clamp) · 여성 W/직책 배지는 번호 우측 · **신청자=라임 / 수동 선정=청록**(registrations의 confirmed/waiting = `appliedIds`로 구분) · **한 줄 4명**(3→4열, 이름 글씨 축소). → tab-attend.js.
- **팀 만들기 = 결과 기본 + 단계 마법사**: 팀 탭 기본은 **편성 결과**(use-team.js 기본 `results`), [생성]/[수정]으로 **모임 생성처럼 단계 마법사**(①대상 ②팀수·옵션 ③편성). ① 대상 카드 1개로 통합(대상 인원 + 남/여 인라인 + 등급칩 '숫자·인원만' + 제외 목록을 **남(위)/여(아래)·높은등급(1) 앞** 정렬) · 상단 '결과' 버튼 제거→하단 **이전** 버튼으로 통합(1단계 이전=결과) · 옵션 버튼(커플/중복회피) 좌우 축소 · **편성 결과 그리드 팀수별 열**(3팀=3열·4팀=2열·5·6팀=3열). → tab-team.js, use-team.js.

## 다음에 할 일 (집/다음 세션)
- 팀 마법사 **3단계(편성 결과)를 마법사 안 미리보기+확정**으로 넣을지 (지금은 편성 시 결과 화면으로 점프) — 사용자 확인 대기.
- **4열 명단** 실기기 확인: 이름 4글자·상단 배지(W·직책)가 좁으면 번호원/배지 크기 더 축소.
- (지난 잔여) 매치 단순화 실기기 검증 · lab-*.html 정리 · 홈 카드에서도 담당자 불참 시 교체팝업.

## 오늘 낮 세션 (2026-07-02, 맥, v1.23.1 → v1.24.1, 배포됨)
