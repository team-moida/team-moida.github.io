...
| 항목 | 내용 |
|------|------|
| 파일 | `tab-attend-setup.js` (신규) |
| 절감 | -171줄 |
| 작업 | 선정 탭 JSX 전체 (모임 설정 + 회원 선정 + 게스트 추가) 컴포넌트화 |

### 7단계 — 출석 탭 분리 ✅ 완료

| 항목 | 내용 |
|------|------|
| 파일 | `tab-attend-main.js` (신규) |
| 절감 | -235줄 |
| 작업 | 출석 탭 JSX 전체 (CurrentTimeClock + 팀별 카드 + 명단 + 대기자 + 액션 버튼) 컴포넌트화 |

### 8단계 — 모달 묶음 분리 ✅ 완료

| 항목 | 내용 |
|------|------|
| 파일 | `modals-attend.js` (신규) |
| 절감 | -242줄 |
| 작업 | makeCheckInHandlers + CheckInModal, makeNotifHandlers + NotifModal, GuestModal, HistoryEditModal, AlertModal 외부화 |
...
| 항목 | 내용 |
|------|------|
| 파일 | `handlers-attend-session.js` (신규) |
| 절감 | -313줄 |
| 작업 | toggleTestMode, saveAndResetCurrent, toggleParticipant, handleAddGuest 등 14개 핸들러 makeAttendSessionHandlers 팩토리로 분리 |
